...
public class MockedWebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/update_comment").hasRole("USER").and().httpBasic().and().exceptionHandling().accessDeniedPage("/access-denied-error-page");
		http.authorizeRequests().antMatchers("/update_details").hasRole("ADMIN").and().httpBasic().and().exceptionHandling().accessDeniedPage("/access-denied-error-page");
